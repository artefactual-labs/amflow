...
func (w Workflow) DOT() ([]byte, error) {
	...
	n.AddNode(initiator)
	for _, vertex := range w.vertices[watchedDirNodeType] {
		if !vertex.(*VertexWatcheDir).isInitiator() {
			continue
		}
		n.SetEdge(n.NewEdge(initiator, vertex))
	}

	// Remove ignored vertices - tricky because n doesn't carry enough information.
	// Improvable but ok for now.
	iter := n.Nodes()
	for iter.Next() {
		id := iter.Node().ID()
		for _, mp := range w.vertices {
			for amid, v := range mp {
				if v.ID() != id {
					continue
				}
